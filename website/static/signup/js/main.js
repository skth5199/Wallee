...
(function ($) {
    ...
    $('.validate-form').on('submit',function(){
        ...
        for(var i=0; i<input.length; i++) {
            ...
            if(validate(input[i],input[2]) == false){
    ...
    function validate (input,input1) {
        if($(input).attr('type') == 'email' || $(input).attr('name') == 'email') {
            ...
            if($(input).val().trim().match("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,14})") == null) {
                return false;
            }
        }
        else if($(input).attr('type') == 'password' || $(input).attr('name') == 'password2') {
            if($(input).val(value) != $(input1).val(value)) {
                return false;
            }
        }
        else {
